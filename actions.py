...
class Actions:
    ...
    def attack(self, hero, enemy, heroTriggered):
        if heroTriggered:
    ...
    def heal(self, hero, enemy, heroTriggered, ammount):
        if heroTriggered:
            hero.heal(ammount)
        else:
            enemy.heal(ammount)

    def recover(self, hero, enemy, ammount):
        hero.recover(ammount)


    def useItem(self, hero, enemy):
        while True:
            item = self.chooseItem(hero)
        ...
        for elements in itemAttributes:
            if elements == "Damage":
                self.attack(hero, enemy, True)
            if elements == "Heal":
                self.heal(hero, enemy, True)
    def tryRun(self):
        x=3

    def chooseItem(self, hero):
